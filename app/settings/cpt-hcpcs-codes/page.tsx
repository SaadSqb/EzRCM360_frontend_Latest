...
export default function CptHcpcsCodesPage() {
  ...
  const handleStatusChange = async (row: CptHcpcsCodeDto, activeValue: number) => {
    if (!canUpdate) return;
    setStatusUpdatingId(row.id);
    try {
      const payload: CreateCptHcpcsCodeCommand = {
        code: row.code,
        shortDescription: row.shortDescription,
        longDescription: row.longDescription ?? null,
        codeType: row.codeType,
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col px-6">
            ...
            <Table className="min-w-[900px] table-fixed">
              <TableHead>
                <TableRow>
                  ...
                  <TableHeaderCell className="w-[260px] min-w-[260px]">Short description</TableHeaderCell>
                  <TableHeaderCell className="w-[120px] min-w-[120px]">Code type</TableHeaderCell>
                  <TableHeaderCell className="w-[100px] min-w-[100px]">Add-on</TableHeaderCell>
      ...
      <Modal
        ...
        title={editId ? "Edit code" : "Add code"}
        size="lg"
        position="right"
        footer={
          <ModalFooter
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div>
              ...
              <select
                value={form.codeType}
                onChange={(e) => setForm((f) => ({ ...f, codeType: Number(e.target.value) }))}
                className="w-full rounded-[5px] border border-input px-3 py-2 text-sm focus:outline-none focus:ring-0 focus-visible:outline-none focus-visible:ring-0"
              >
            ...
            <div className="flex items-center gap-4">
              <label htmlFor="cpt-add-on" className="inline-flex w-fit cursor-pointer items-center gap-2">
                <input
                  id="cpt-add-on"
                  type="checkbox"
              ...
              <label htmlFor="cpt-active" className="inline-flex w-fit cursor-pointer items-center gap-2">
                <input
                  id="cpt-active"
                  type="checkbox"
      ...
      <ConfirmDialog
        ...
        title="Delete selected CPT/HCPCS codes"
        message={`Are you sure you want to delete ${selectedIds.size} CPT/HCPCS code(s)? This action cannot be undone.`}
